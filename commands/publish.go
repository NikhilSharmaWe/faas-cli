...
func runPublish(cmd *cobra.Command, args []string) error {
	...
	errors := publish(&services, parallel, shrinkwrap, quietBuild, mountSSH, remoteBuilder)
	if len(errors) > 0 {
...
func publish(services *stack.Services, queueDepth int, shrinkwrap, quietBuild, mountSSH bool, remoteBuilder string) []error {
	startOuter := time.Now()
